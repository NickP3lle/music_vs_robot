...
template <class T> ptr<T>::ptr(T *ptr_t) : data(ptr_t), refCount(ptr_t ? new unsigned int(1) : nullptr) {}

template <class T> ptr<T>::ptr(const T &t) : ptr(new T(t)) {}

template <class T> ptr<T>::ptr(const ptr &Ptr) : data(Ptr.data), refCount(Ptr.refCount) {
    if (refCount)
        ++(*refCount);
}
