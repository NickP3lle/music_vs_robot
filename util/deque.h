...
template <class T> deque<T>::deque(u32 cap) : first(cap == 0 ? nullptr : new T[cap]), capacity(cap), size(0), actual(0) {}
...
template <class T> deque<T>::deque(const deque &d) : first(copy(d)), capacity(d.capacity), size(d.size), actual(d.actual) {}
...
template <class T> T &deque<T>::operator[](u32 index) const { return first[(actual + index) % capacity]; }

// ciclo for sulla deque
template <class T> template <typename S> const deque<T> &deque<T>::iter(S lambda, u32 begin) const {
    return iter(lambda, begin, size);
}

// ciclo for sulla deque
template <class T> template <typename S> const deque<T> &deque<T>::iter(S lambda, u32 begin, u32 end) const {
    for (; begin < end; begin++)
        lambda((*this)[begin]);
    return *this;
}
