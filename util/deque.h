...
template <typename T>
deque<T>::deque(u32 capacity) : f(capacity == 0 ? nullptr : new T[capacity]), c(capacity), s(0), a(0) {}

/// costruttore di copia
template <typename T> deque<T>::deque(const deque &d) : f(copy(d)), c(d.c), s(d.s), a(0) {}
...
template <typename T> template <typename S> const deque<T> &deque<T>::iter(S lambda) const {
    for (u32 i = 0; i < s; i++)
        lambda((*this)[i]);
    return *this;
}

template <typename T> template <typename S, typename U> deque<U> deque<T>::iter(S lambda) const {
    deque<U> d;
    for (u32 i = 0; i < s; i++)
...
template <typename T> template <typename S> deque<T *> deque<T>::filter(S lambda) const {
    deque<T *> d;
