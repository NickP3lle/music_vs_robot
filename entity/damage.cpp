...
void DamageBullet::accept(VisitorInterface *visitor) const { visitor->visitDamageBullet(); }
...
void DamageSlow::accept(VisitorInterface *visitor) const { visitor->visitDamageSlow(); }
...
void DamageWave::accept(VisitorInterface *visitor) const { visitor->visitDamageWave(); }

/// DamagePlayer
DamagePlayer::DamagePlayer(u32 bd, u32 sd, u32 wd) : DamageBullet(bd), DamageSlow(sd), DamageWave(wd) {}
...
void DamagePlayer::operator/(const u32 &d) {
    *this = DamagePlayer(DamageBullet::damage() / d, getSlow(), DamageWave::damage() / d);
}
...
DamageEnemy DamageEnemy::operator+(const DamageEnemy &other) const { return DamageEnemy(d + other.d); }

DamageEnemy DamageEnemy::operator+(const u32 &u) const { return DamageEnemy(d + u); }
