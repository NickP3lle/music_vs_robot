...
void Playground::playerAttack(u32 col) {
    ...
    for (; row < ROWS; row++) {
        ...
        if (col > 3 && player[row][col - 4] && dynamic_cast<const Drum *>(&*player[row][col - 4]))
...
void Playground::notifyRobotObservers(int row, int col) {
    ...
    // bug?
    getInstance()->enemyGet(row, col).iter([obs = instance->observers, row, col](const RobotWTool *r) {
        for (auto &ob : obs)
...
void Playground::playerLoad(u32 row, u32 col, MusicInstruments *mi) {
    player[row][col] = std::unique_ptr<MusicInstruments>(mi->clone());
    notifyMusicObservers(row, col);
}