...
bool Test_Sample_attack() {
  u32 l = randomInt(100, 50);
  Sample sample(l);
  DamagePlayer *damage = sample.attack();
  bool testPassed =
      (damage != nullptr) &&
      (damage->damage() == (SAMPLE_POWER + (l - 1) * SAMPLE_POWER_INCREASE));
  delete damage;
  return testPassed;
}
...
bool Test_ThreeColumn_attack() {
  ...
  ThreeColumn threeColumn(l);
  DamagePlayer *damage = threeColumn.attack();

  bool testPassed = (damage != nullptr) && (damage->damage() == a);
  delete damage;
...
bool Test_DoubleLife_attack() {
  ...
  DoubleLife doubleLife(l);
  DamagePlayer *damage = doubleLife.attack();
...
bool Test_ThreeRow_attack() {
  ...
  ThreeRow threeRow(l);
  DamagePlayer *damage = threeRow.attack();
...
bool Test_SlowDown_attack() {
  ...
  SlowDown slowDown(l);
  DamagePlayer *damage = slowDown.attack();
  bool testPassed = (damage != nullptr) && (damage->damage() == a);
  delete damage;
