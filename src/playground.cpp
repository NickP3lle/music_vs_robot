...
void Playground::playerAttack(u32 col) {
  for (u32 i = 0; i < ROWS; i++) {
    if (player[i][col].isNone())
      continue;
    if (!dynamic_cast<const Drum *>(&player[i][col].get()))
      damage[i][0] += player[i][col].get().attack();
    if (dynamic_cast<const Trumpet *>(&player[i][col].get())) {
      if (i > 0)
    ...
    if (dynamic_cast<const Violin *>(&player[i][col].get()))
      slowDown[i]++;
    if (i > 5 && dynamic_cast<const Trumpet *>(&player[i][col - 4].get()))
      damage[i][1] -= player[i][col - 4].get().attack();
  }
}
...
void Playground::enemyAttack(u32 col) {
  for (u32 row = 0; row < ROWS; row++) {
    ...
    iterRobot(row, col, [&danni](deque<RobotWTool> &d) {
      d.iter().map([&danni](RobotWTool &e) { danni += e.attack(); });
    });
...
u32 Playground::moveRobot(u32 row, u32 col, RobotWTool &r) {
  return col - std::min(nearestPlayer(row, col),
                        r.move() / FRAME_COLUMNS / (slowDown[row]-- ? 2 : 1));
}
