            ...
            while (enemy[row][col].len() > i) {
                ...
                if (col / FRAME_COLUMNS > damagePos && move_to / FRAME_COLUMNS <= damagePos &&
                    (enemy[row][col][i].takeDamage(MusicInstruments::damages[row][1]) ||
                     enemy[row][col][i].takeDamage(MusicInstruments::damages[row][0]))) {
                ...
                std::cout << "move to: " << move_to << std::endl;
            }
        }
    }
    return false;
...
const MusicInstruments *Playground::playerGet(u32 row, u32 col) const { return player[row][col].getPtr(); }
    ...
    for (u32 i = 0; i < ROWS; i++) {
        for (u32 j = 0; j < COLUMNS; j++) {
            if (!isEmpty(i, j)) {
                ...
                data += player[i][j].get_mut().saveData();
            } else {
                data += "null";
            }
