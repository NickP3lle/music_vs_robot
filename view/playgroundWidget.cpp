...
PlaygroundWidget::PlaygroundWidget(QWidget *parent)
    : QWidget(parent), backButton(new QPushButton("Back", this)),
      ...
      saxophoneButton(
          new InstrumentButton(new DoubleLife(), "Saxophone ", this)),
      fluteButton(new InstrumentButton(new Sample(), "Flute ", this)),
      levelUpButton(new QPushButton("Level Up", this)),
...
bool PlaygroundWidget::getFocus() const {
...
void PlaygroundWidget::showUpdatePrice() {
  ...
  if (!g->isEmpty(hasFocus.row, hasFocus.col)) {
    levelUpButton->setText(
        "Level Up: " +
        QString::number(g->get(hasFocus.row, hasFocus.col).getCost()) + "$");
  } else {
